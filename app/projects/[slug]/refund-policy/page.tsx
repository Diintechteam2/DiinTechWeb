...
export default async function ProjectRefundPolicy({ params }: PageProps) {
  ...
  if (!project || !project.refundPolicy?.enabled) {
    notFound()
  }

  const refundContent = project.refundPolicy.content
    ...
    <main className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10 pt-32 pb-24">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-8 animate-slide-up">
          ...
          <ChevronRight className="w-4 h-4" />
          <span className="text-primary/80">Refund Policy</span>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            <div className="prose prose-invert max-w-none space-y-12">
              ...
              {/* Eligibility */}
              {refundContent.eligibility && (
                <section>
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-8 h-8 rounded-lg bg-blue-500/20 border border-blue-500/30 flex items-center justify-center text-blue-400">
                      <Shield className="w-4 h-4" />
                    </div>
                    <h2 className="text-2xl font-bold text-foreground m-0">2. Refund Eligibility Guidelines</h2>
                  </div>
              ...
              {/* Timeline */}
              {refundContent.timeline && (
                <section>
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-8 h-8 rounded-lg bg-purple-500/20 border border-purple-500/30 flex items-center justify-center text-purple-400">
                      <Clock className="w-4 h-4" />
                    </div>
                    <h2 className="text-2xl font-bold text-foreground m-0">3. Refund Timeline & Settlement</h2>
                  </div>
                  <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                    {refundContent.timeline}
                  </p>
              ...
              {/* Process */}
              {refundContent.process && (
                <section>
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-8 h-8 rounded-lg bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center text-emerald-400">
                      <Mail className="w-4 h-4" />
                    </div>
                    <h2 className="text-2xl font-bold text-foreground m-0">4. How to Request a Refund</h2>
                  </div>
